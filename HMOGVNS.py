            ...
            for tag in ["6CW", "CM"]:
                ...
                if os.path.exists(file_path):
                    with open(file_path, "r") as file:
                        ...
                        if len(lines) < 4:
                            print(f"Incomplete energy price file (missing lines): {file_name}")
                            continue
                        ...
                        if not end_vector:
                            print(f"Missing end vector in file: {file_name}")
...
def machine_sequence_swap_random(schedule, machines, jobs, processing_times, time_horizon):
    new_cmax = time_horizon + 1
    attempts = 0

    while new_cmax > time_horizon and attempts < 10:
        attempts += 1
        new_schedule = [list(machine) for machine in schedule]
        machine1, machine2 = random.sample(range(machines), 2)
        ...
        new_cmax = calculate_cmax(new_schedule, machines, jobs, processing_times)
...
def machine_sequence_swap_three_random(schedule, machines, jobs, processing_times, time_horizon):
    new_cmax = time_horizon + 1
    attempts = 0
    new_schedule = [list(machine) for machine in schedule]
    ...
    while new_cmax > time_horizon and attempts < 10:
        attempts += 1
        new_schedule = [list(machine) for machine in schedule]
        machine1, machine2, machine3 = random.sample(range(machines), 3)

        job_indexes1 = [job[0] for job in new_schedule[machine1]]
        job_indexes2 = [job[0] for job in new_schedule[machine2]]
        job_indexes3 = [job[0] for job in new_schedule[machine3]]

        for i, job in enumerate(new_schedule[machine1]):
            new_schedule[machine1][i] = (job_indexes2[i], job[1])

        for i, job in enumerate(new_schedule[machine2]):
            new_schedule[machine2][i] = (job_indexes3[i], job[1])

        for i, job in enumerate(new_schedule[machine3]):
            new_schedule[machine3][i] = (job_indexes1[i], job[1])
...
def sequence_inversion_random(schedule, machines, jobs, processing_times, time_horizon):
    new_cmax = time_horizon + 1
    attempts = 0
    new_schedule = [list(machine) for machine in schedule]
    ...
    while new_cmax > time_horizon and attempts < 10:
        ...
        p1, p2 = sorted(random.sample(range(len(job_indexes)), 2))
...
def vns(schedule, machines, jobs, processing_times, energy_consumption_rates, time_periods_end, energy_prices, time_periods_start, max_iter, time_horizon):
    shake_neighborhoods = [
        machine_sequence_swap_random,
        sequence_inversion_random,
        machine_sequence_swap_three_random
    ]

    best_schedule = [list(machine) for machine in schedule]
    shake_schedule = [list(machine) for machine in schedule]
    best_fitness = evaluate(best_schedule, processing_times, energy_consumption_rates, time_periods_end, time_periods_start, energy_prices, machines, jobs)
    ...
    while iteration < max_iter:
        ...
        while neighborhood_index < len(shake_neighborhoods):
            shaking = shake_neighborhoods[neighborhood_index]
            temp = [list(machine) for machine in best_schedule]
            shake_schedule = shaking(temp, machines, jobs, processing_times, time_horizon)

            if not is_schedule_feasible(shake_schedule, processing_times):
                continue

            temp2 = [list(machine) for machine in shake_schedule]
            shake_fitness = evaluate(shake_schedule, processing_times, energy_consumption_rates, time_periods_end, time_periods_start, energy_prices, machines, jobs)

            if shake_fitness[0] > time_horizon:
                continue

            new_schedule, new_fitness, glob = VND(temp2, processing_times, machines, jobs, energy_consumption_rates, time_periods_end, energy_prices, time_periods_start, time_horizon)
            ...
            temp4 = [list(machine) for machine in new_schedule]
            new_schedule2 = tec_reducer(temp4, machines, jobs, processing_times, time_periods_start, time_periods_end, energy_prices)
            ...
            if dominates(new_fitness, best_fitness):
                best_schedule = [list(machine) for machine in new_schedule]
...
def process_instance(instance, energy_config, consumption_config):
    ...
    if jobs <= 60:
        ...
        for p in p_values:
            ...
            cmax, tec = evaluate(new_schedule, processing_times, energy_consumption_rates, time_periods, time_periods_start, energy_prices, machines, jobs)
            if cmax < best_cmax:
                best_cmax = cmax
                best_p = p
        p = best_p
    ...
    new_schedule = nfs_heuristic(machines, jobs, processing_times, p)
    fitness = evaluate(new_schedule, processing_times, energy_consumption_rates, time_periods, time_periods_start, energy_prices, machines, jobs)
    print(f"Initial solution fitness: {fitness}")

    updated_individual, updated_fitness, all_solutions = vns(
        new_schedule, machines, jobs, processing_times, energy_consumption_rates,
        time_periods, energy_prices, time_periods_start, 400, time_horizon
    )
...
def process_instance_parallel(instance, instance_idx, config_type, batch_dir):
    print(f"Processing Instance {instance_idx} with {instance['machines']} machines and {instance['jobs']} jobs (Config: {config_type}).")
    start_time = time.time()

    all_solutions, initial_solution = process_instance(instance, "6CW", config_type)

    end_time = time.time()
    execution_time = end_time - start_time

    fitness_values = [fitness for fitness in all_solutions]
    pareto_front = get_pareto_front(fitness_values)
    pareto_front.sort(key=lambda x: x[1])

    os.makedirs(CSV_DIR, exist_ok=True)

    instance_csv_path = os.path.join(CSV_DIR, f"M{instance['machines']}_J{instance['jobs']}_config_6CW_{instance_idx % 10}.csv")
    with open(instance_csv_path, mode='w', newline='') as csvfile:
...
def main(instances_data):
    ...
    for machines, instances in grouped_instances.items():
        selected_instances.extend(instances[0:10])
    ...
    for instance_idx, instance in enumerate(selected_instances):
        for config_type in configurations:
            p = Process(target=process_instance_parallel, args=(instance, instance_idx, config_type, batch_dir))
            p.start()
            processes.append(p)
